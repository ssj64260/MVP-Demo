...
import com.cxb.mvp_project.utils.DisplayUtil;
...
public class FamilyTreeView2 extends ViewGroup {
    ...
    private FamilyBean mFamilyBean;//我的
    private FamilyBean mMySpouse;//配偶
    private FamilyBean mMyFather;//父亲
    private FamilyBean mMyMother;//母亲
    private List<FamilyBean> mMyBrothers;//兄弟姐妹
    private List<FamilyBean> mMyChildren;//子女
    private FamilyBean mMyFosterFather;//养父
    private FamilyBean mMyFosterMother;//养母
    ...
    public FamilyTreeView2(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTransform = new GlideCircleTransform(context);
...
//        mPaint.setPathEffect(new DashPathEffect(new float[]{mLineWidthPX, mLineWidthPX * 4}, 0));
    ...
    private void initData(FamilyBean family) {
        this.mFamilyBean = family;
        mFamilyBean.setSelect(true);
        mMySpouse = mFamilyBean.getSpouse();
        mMyFather = mFamilyBean.getFather();
        mMyMother = mFamilyBean.getMother();
        mMyBrothers = mFamilyBean.getBrothers();
        mMyChildren = mFamilyBean.getChildren();
        mMyFosterFather = mFamilyBean.getFosterFather();
        mMyFosterMother = mFamilyBean.getFosterMother();
    }
    ...
    private void initWidthAndHeight() {
        ...
        if (mMyChildren != null) {
            ...
            for (int i = 0; i < mMyChildren.size(); i++) {
                final FamilyBean child = mMyChildren.get(i);
                final List<FamilyBean> grandChildrenList = child.getChildren();
    ...
    private void initView() {
        mMineView = createFamilyView(mFamilyBean);
        if (mMySpouse != null) {
        ...
        if (mMyFather != null) {
            ...
            final FamilyBean myPaternalGrandFather = mMyFather.getFather();
            final FamilyBean myPaternalGrandMother = mMyFather.getMother();
        ...
        if (mMyMother != null) {
            ...
            final FamilyBean myMaternalGrandFather = mMyMother.getFather();
            final FamilyBean myMaternalGrandMother = mMyMother.getMother();
        ...
        if (mMyFosterFather != null) {
            ...
            final FamilyBean myFPGrandFather = mMyFosterFather.getFather();
            final FamilyBean myFPGrandMother = mMyFosterFather.getMother();
        ...
        if (mMyFosterMother != null) {
            ...
            final FamilyBean myFMGrandFather = mMyFosterMother.getFather();
            final FamilyBean myFMGrandMother = mMyFosterMother.getMother();
        ...
        mBrothersView.clear();
        if (mMyBrothers != null) {
            for (FamilyBean family : mMyBrothers) {
        ...
        mChildrenView.clear();
        if (mMyChildren != null) {
            for (FamilyBean family : mMyChildren) {
                mChildrenView.add(createFamilyView(family));
                final FamilyBean childSpouse = family.getSpouse();
                if (childSpouse != null) {
                    mChildSpouseView.add(createFamilyView(childSpouse));
                }

                final List<FamilyBean> grandChildrens = family.getChildren();

                if (grandChildrens != null && grandChildrens.size() > 0) {
                    for (FamilyBean childFamily : grandChildrens) {
    ...
    private View createFamilyView(FamilyBean family) {
        ...
        if (mMineView != null) {
            ...
            if (mChildrenView != null && mChildrenView.size() > 0) {
                ...
                for (int i = 0; i < childCount; i++) {
                    ...
                    final FamilyBean myChild = mMyChildren.get(i);
                    final FamilyBean myChildSpouse = myChild.getSpouse();
                    final List<FamilyBean> myGrandChildren = myChild.getChildren();
...
//        super.onDraw(canvas);
    ...
    private void drawChildrenLine(Canvas canvas) {
        if (mMyChildren != null && mMyChildren.size() > 0) {
            ...
            for (int i = 0; i < childrenViewCount; i++) {
                ...
                final FamilyBean childSpouse = mMyChildren.get(i).getSpouse();
                if (childSpouse != null) {
                ...
                final List<FamilyBean> grandChildren = mMyChildren.get(i).getChildren();
    ...
    public void setFamilyMember(FamilyBean mFamilyBean) {
        recycleAllView();
        initData(mFamilyBean);
        initWidthAndHeight();
        initView();
        invalidate();
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                break;
            case MotionEvent.ACTION_MOVE:
                final int distanceX = Math.abs((int) ev.getX() - mLastInterceptX);
                final int distanceY = Math.abs((int) ev.getY() - mLastInterceptY);
